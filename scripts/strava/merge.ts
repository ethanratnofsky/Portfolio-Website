...
export interface Snapshot {
    [stravaId: string]: SnapEntry;
}
export type DraftMatch = Record<string, any> & {
    stravaId?: number;
    score: [number, number];
    goals: number;
    assists: number;
...
const snapOf = (m: DraftMatch): SnapEntry => ({
    ...
    goals: m.goals,
    assists: m.assists,
    goalsIsMinimum: Boolean(m.goalsIsMinimum),
});
const sameSnap = (a: SnapEntry, b: SnapEntry) =>
    ...
    a.goals === b.goals &&
    a.assists === b.assists &&
    a.goalsIsMinimum === b.goalsIsMinimum;
const label = (m: DraftMatch) =>
    `${m.date} ${m.teamId ?? m.guest?.team ?? "[Unknown team]"}`;
...
export function mergeImports(
    ...
    for (const d of drafts) {
        ...
        if (!stravaChanged) {
            res.skipped.push(`unchanged ${label(repo)}`);
            continue;
        }
        if (repoChanged) {
            res.conflicts.push(
                `conflict ${label(repo)}: repo ${repo.score[0]}–${repo.score[1]} vs Strava ${d.score[0]}–${d.score[1]} (kept repo)`
            );
            continue;
        }
        matches[idx] = { ...repo, ...d };
